...
<div class="abf-frame">
  ...
  <br/>
    ...
    <div class="abf-topline"><div>
      ...
      <div style="text-align:center; height:186px"><img src="https://gtechz.implogix.com/api/V10/processing.gif" style="display: inline;height:200px;width:230px;margin-top: -5%;" alt="official QR code for payment"> </div>
    </div>
    <h4 style="text-align:center;color:#dc3545 !important;">You will be redirected to merchant Websites in <span id="timer" style="color:#dc3545 !important;">00:30 seconds</span>.</h4>
      <div class="abf-list">
        <div class="abf-list-item">
          ...
          <div class="abf-value"><span class="abf-totalbtc"><?php echo base64_decode($_GET['amount']); ?></span> THB</div>
        </div>
        <div class="abf-list-item">
          <div class="abf-label">BackCode:</div>
          <div class="abf-value"><span class="abf-totalbtc"></span><?php echo base64_decode($_GET['ref_bank_code']); ?></div>
        </div>
        ...
        function updateTimer() {
            ...
            if (remainingTime === 0) {
                ...
                <?php
                    ...
                    $query2 = "SELECT price,customer_email,payin_request_id,payin_notify_url,payin_success_url,payin_error_url,orderid,created_date,orderstatus FROM `m_payin` WHERE payin_request_id='$payin_request_id' ";
                    ...
                    if(!empty($row)){
                        ...
                        if(!empty($redirecturl)){
                            $callbackURL=$redirecturl.'?pt_transactionId='.base64_encode($row['payin_request_id']).'&pt_email='.base64_encode($row['customer_email']).'&pt_reference='.base64_encode($row['payin_request_id']).'&pt_amount='.base64_encode($row['price']).'&pt_timestamp='.base64_encode($row['created_date']).'&pt_status='.base64_encode($row['orderstatus']);
